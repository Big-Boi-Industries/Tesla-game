...
public class BuildTeslaCoil : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Part.name.Contains("1")) { PickedUp = false; Placed1 = true; Part.transform.rotation = new Quaternion(0, 0, 0, 0); Part.GetComponent<Rigidbody>().constraints = RigidbodyConstraints.FreezeAll; Part.transform.position = new Vector3(WorkBench.transform.position.x, WorkBench.transform.position.y + 0.75f, WorkBench.transform.position.z); Part = null; }
            if (Part.name.Contains("2")) { PickedUp = false; Placed2 = true; Part.transform.rotation = new Quaternion(0, 0, 0, 0); Part.GetComponent<Rigidbody>().constraints = RigidbodyConstraints.FreezeAll; Part.transform.position = new Vector3(WorkBench.transform.position.x, WorkBench.transform.position.y + 1.25f, WorkBench.transform.position.z); Part = null; }
